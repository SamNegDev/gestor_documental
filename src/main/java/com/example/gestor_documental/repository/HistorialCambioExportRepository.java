...
public interface HistorialCambioExportRepository extends Repository<HistorialCambio, Long> {

    @Query("""
            select h from HistorialCambio h
            left join fetch h.usuario
            where h.expediente.id = :expedienteId
              and (:desde is null or h.fechaCambio >= :desde)
              and (:hastaExclusiva is null or h.fechaCambio < :hastaExclusiva)
              and (
                :categoria is null
            ...
            """)
    Slice<HistorialCambio> buscarParaExportar(
