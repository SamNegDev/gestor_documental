...
public class FacturaHoldedApiController {
 ...
 @PostMapping(value="/confirmar",consumes=MediaType.MULTIPART_FORM_DATA_VALUE) public AnalisisFacturaArchivoResponse confirmar(@RequestParam Long facturaId,@RequestParam ModalidadFacturacion modalidad,@RequestParam(required=false) @DateTimeFormat(iso=DateTimeFormat.ISO.DATE) LocalDate periodoDesde,@RequestParam(required=false) @DateTimeFormat(iso=DateTimeFormat.ISO.DATE) LocalDate periodoHasta,@RequestParam List<Long> expedienteIds,@RequestParam MultipartFile archivo,Authentication auth)throws IOException{currentUserService.requireAdmin(auth);return analisisService.confirmar(facturaId,modalidad,periodoDesde,periodoHasta,expedienteIds,archivo);}
