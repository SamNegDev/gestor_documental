...
public class ResumenFinalizadosDiarioService {
 ...
 @Value("${app.daily-finalized.enabled:false}") private boolean enabled;
 ...
 @Scheduled(cron="${app.daily-finalized.cron:0 0 17 * * *}",zone="${app.daily-finalized.zone:Atlantic/Canary}")
 public void enviarProgramado(){if(enabled){Resultado r=enviarDelDia();log.info("Aviso de finalizados: {} correos, {} expedientes",r.correos(),r.expedientes());}}
 Resultado enviarDelDia(){ZoneId z=ZoneId.of(zone);LocalDate d=LocalDate.now(z);var cambios=historialCambioRepository.findFinalizacionesExpedienteEntre(d.atStartOfDay(),d.plusDays(1).atStartOfDay());Map<Long,Grupo> grupos=new LinkedHashMap<>();
  ...
  int enviados=0,total=0;List<String> avisos=new ArrayList<>();for(var g:grupos.values()){if(!permite(g.cliente))continue;var items=List.copyOf(g.items.values());var r=correoService.enviarHtml(g.cliente.getEmail(),asunto(d,items.size()),html(g.cliente,d,items),texto(g.cliente,d,items),copias(g.cliente.getEmail()),LOGO);if(r.exito()){enviados++;total+=items.size();}else avisos.add(g.cliente.getEmail()+": "+r.error());}return new Resultado(enviados,total,List.copyOf(avisos));}
 private boolean permite(Cliente c){if(c==null||!StringUtils.hasText(c.getEmail()))return false;var p=c.getPreferenciaCanal()==null?PreferenciaCanalCliente.AMBOS:c.getPreferenciaCanal();return p==PreferenciaCanalCliente.EMAIL||p==PreferenciaCanalCliente.AMBOS;}
