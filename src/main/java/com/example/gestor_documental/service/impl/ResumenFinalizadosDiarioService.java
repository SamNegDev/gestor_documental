...
public class ResumenFinalizadosDiarioService {
 ...
 private Resultado enviar(LocalDate d,List<HistorialCambio> cambios){Map<Long,Grupo> grupos=new LinkedHashMap<>();
  ...
  int enviados=0,total=0;
  List<String> avisos=new ArrayList<>();
  for(var g:grupos.values()){
   if(!permite(g.cliente))continue;
   var items=List.copyOf(g.items.values());
   if(items.isEmpty())continue;
   String destinatario=g.cliente.emailNotificacionesEfectivo();
   var r=correoService.enviarHtml(destinatario,asunto(d,items.size()),html(g.cliente,d,items),
           texto(g.cliente,d,items),g.cliente.getEmailsCopiaNotificaciones(),copias(destinatario),LOGO);
   if(r.exito()){
    enviados++;
    total+=items.size();
   }else{
    avisos.add(destinatario+": "+r.error());
   }
  }
  return new Resultado(enviados,total,List.copyOf(avisos));
 }
 private boolean permite(Cliente c){
  if(c==null||!StringUtils.hasText(c.emailNotificacionesEfectivo()))return false;
  var p=c.getPreferenciaCanal()==null?PreferenciaCanalCliente.AMBOS:c.getPreferenciaCanal();
  return p==PreferenciaCanalCliente.EMAIL||p==PreferenciaCanalCliente.AMBOS;
 }
