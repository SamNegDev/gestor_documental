...
import type { AnalisisFactura, EstadoComprobante, FacturasPage, ModalidadFacturacion } from "../types";
...
export function confirmarFactura(input:{facturaId?:number;modalidad:ModalidadFacturacion;periodoDesde?:string;periodoHasta?:string;expedienteIds:number[];archivo:File}){const form=new FormData();if(input.facturaId)form.append("facturaId",String(input.facturaId));form.append("modalidad",input.modalidad);if(input.periodoDesde)form.append("periodoDesde",input.periodoDesde);if(input.periodoHasta)form.append("periodoHasta",input.periodoHasta);input.expedienteIds.forEach(id=>form.append("expedienteIds",String(id)));form.append("archivo",input.archivo);return apiPostForm<AnalisisFactura>("/api/facturas/confirmar",form)}