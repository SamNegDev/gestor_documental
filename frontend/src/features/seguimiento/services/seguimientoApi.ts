...
import type { NotificacionPreview, NotificacionResultado, ResumenDiarioResponse, SeguimientoConfig, SeguimientoPage } from "../types";
...
export const getNotificacionWhatsappPreview=(id:number)=>apiGet<NotificacionPreview>(`/api/seguimiento-clientes/${id}/notificacion-whatsapp-preview`);
...
export const notificarSeguimientoWhatsapp=(id:number,body:{mensaje:string})=>apiPostJson<NotificacionResultado>(`/api/seguimiento-clientes/${id}/notificar-whatsapp`,body);
export const enviarListadoIncidencias=(clienteId?:string)=>apiPostJson<ResumenDiarioResponse>(`/api/admin/resumen-diario-tramites/incidencias/enviar-masivo${clienteId ? `?clienteId=${clienteId}` : ""}`,{});
